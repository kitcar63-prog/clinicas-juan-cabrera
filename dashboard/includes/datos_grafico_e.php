...
$seleccionTratamiento ="Todos";
$seleccionFecha = "Última semana";
$seleccionTratamiento = $_POST["seleccionTratamiento"];
...
while ($row = $result->fetch_assoc()) {
  ...
  if (strpos($tratamiento, 'Varicosa') !== false) {
    ...
    $tratamientook=['Úlcera Varicosa','Varices','Hemorroides'];
      ...
      if(($seleccionTratamiento==$tratamiento || $seleccionTratamiento=="Todos") && $clinica<>""){  
    if (isset( $datagrafico1[$clinica])) {        
       $datagrafico1[$clinica]++;
     } else {
        // Si la clínica no existe, agregarla al arreglo con contador 1
         $datagrafico1[$clinica] = 1;
    }  
     }
