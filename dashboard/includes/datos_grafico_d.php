...
//GRAFICO A.1
$sql = "SELECT clinica,tratamiento,contactar_por FROM contacto WHERE fecha >= '$fecha_desde 00:00:00' AND fecha <= '$fecha_actual 23:59:59' ".$consultaRrss;
...
while ($row = $result->fetch_assoc()) {
    ...
    // Desencriptar el campo 'clinica'
       $contactar=decrypt($row['contactar_por']);
    if($contactar==""){
  ...
  if (strpos($tratamiento, 'Varicosa') !== false) {
    ...
    $tratamientook=['Úlcera Varicosa','Varices','Hemorroides'];
     ...
     if($tratamiento<>"" && $contactar<>"" && ($seleccionClinica==$clinica || $seleccionClinica=="Todas")){      
    if (isset( $datagrafico1[$contactar])) {        
       $datagrafico1[$contactar]++;
     } else {
        // Si la clínica no existe, agregarla al arreglo con contador 1
         $datagrafico1[$contactar] = 1;
    }  
     }
